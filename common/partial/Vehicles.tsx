...
const Tours: FC<ICommonUpcomingEventsProps> = ({ isFluid }) => {
	...
	useEffect(() => {
		...
		listVehicle({ data : {} }, user.token!).then((res:any) => {
				setListData(res);
				setIsLoading(false);
				setIsError(null);
			}
...
setUpcomingEventsEditOffcanvas(!upcomingEventsEditOffcanvas);
	...
	const handleEditAction = (post_data: any) => {
		...
		updateVehicle({ data: postData }, user.token!)
			.then((res) => {
	...
	const handleDeleteAction = (postData: any) => {
		deleteVehicle({ data: postData}, user.token!)
			.then((res) => {
				setListData(res);
		})
		.catch((err:any) => {
				...
				<form onSubmit={handleSubmit((data) => handleEditAction(data))}>
					...
					<div className='row g-4'>
						    ...
						    <div className='col-12'>
								<FormGroup id='type' label='Marka' isFloating>
							...
							<FormGroup id='model' label='Model' isFloating>
								...
								{errors.model && <span>Bu alan gerekli</span>}
							</div>
							<div>
							<FormGroup id='plate' label='Plaka' isFloating>
								...
								{errors.plate && <span>Bu alan gerekli</span>}
							</div>
							<div>
							<FormGroup id='capacity' label='Kapasite' isFloating>
								<Controller name="capacity"
                                            rules={{ required: true }}
                                             control={ control}
                                            render={({ field }) => (
												<Input
								...
								{errors.capacity && <span>Bu alan gerekli</span>}
							</div>
