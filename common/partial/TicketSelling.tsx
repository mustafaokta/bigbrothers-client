...
import {
  postAddTourReservation,
  useDataAgency,
  useDataPaymentMethods,
  useDataHotelList,
  useDataTourList,
  listTourReservation,
...
const List: FC<ICommonUpcomingEventsProps> = ({ isFluid }) => {
  ...
  const {
    data: tourTypeData,
    isLoading: tourTypeIsLoading,
    isError: tourTypeIsError,
  } = useDataTourType();
  ...
  useEffect(() => {
    ...
    if (tourData && selectedTourTypeId) {
      ...
      const filteredTours = tourData.content.filter(
        (tour: any) => tour.type.id == selectedTourTypeId
      );
  ...
  const handleUpcomingEdit = (itm: any) => {
    ...
    let itemm: { [key: string]: any } = {
      ...
      soldDate: itm.mediaPayment
        ? new Date(itm.mediaPayment?.soldDate).toISOString().split("T")[0]
        : "",
    ...
    for (let i = 0; i < itm.customers.length; i++) {
      ...
      itemm[`customerIdentityNumber${i + 1}`] =
        itm.customers[i]["customerIdentityNumber"];
      itemm[`customerPhoneNumber${i + 1}`] =
        itm.customers[i]["customerPhoneNumber"];
      itemm[`customerAddress${i + 1}`] = itm.customers[i]["customerAddress"];
      itemm[`customerDateOfBirth${i + 1}`] = itm.customers[i][
        "customerDateOfBirth"
      ]
        ? itm.customers[i]["customerDateOfBirth"].split("T")[0]
        : "";
  ...
  const handleUpcomingPrint = (itm: any) => {
    ...
    window.open(
      `${process.env.NEXT_PUBLIC_API_HOST}/ticket/${itm.reservationUUID}`,
      "_blank"
    );
  };
  const handleNewItem = () => {
  console.log('userlist : ',userListData.content);
  console.log('user', user);
    ...
    reset({
      ...
      note: "",
      needsTransfer: false,
      mediaType: mediaPaymentTypes[0].id,
      mediaPrice: "",
      mediaPaid: "",
      isSold: false,
      soldDate: "",
  ...
  const handleDeleteAction = (postData: any) => {
    ...
    deleteTourReservation(
      {
        data: {
          reservationType: ["gelen", "giden"],
          reservationId: postData.id,
        },
      },
      user.token!
    )
  ...
  const mediaPaymentTypes = [
    { id: 1, name: "Standart" },
    { id: 2, name: "Ekstra 360" },
  ];
  ...
  sellerCompany =
    process.env.NEXT_PUBLIC_COMPANY_NAME === "Bigbrothers"
      ? sellerCompanyBigbrothers
      : sellerCompanyTigersson;
    ...
    <>
      <Card stretch={isFluid}>
        ...
        <CardBody className="table-responsive" isScrollable={isFluid}>
          <table
            ...
            ref={componentRef as React.RefObject<HTMLTableElement>}
          >
            <thead>
              <tr>
                <td style={{ width: 60 }} />
            ...
            <tbody>
              {dataPagination(items, currentPage, perPage).map((item: any) => (
                <tr key={item.id}>
                  ...
                  <td>
                    {
                      paymentMethodsData.content.filter(
                        (el: any) => el.id == item.paymentMethodId
                      )[0].name
                    }{" "}
                    - {item.currency}
                  </td>
      ...
      >
        <ModalHeader setIsOpen={setNewItemOffcanvas}>
          <OffCanvasTitle id="newRecordIncomingTitle">
            {" "}
            {postType === "update" ? "Düzenle" : "Yeni"}{" "}
          </OffCanvasTitle>
        </ModalHeader>
        <ModalBody>
          <form
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        list={(watch("sellingType") == "office"
                          ? sellerCompany.filter((el: any) => el.id === 1)
                          : sellerCompany
                        ).map((el: any) => ({
              ...
              <div className="col-6 col-md-4">
                <FormGroup id="tourDate" label="Tur Tarih" isFloating>
                  <Controller
                    ...
                    rules={{ required: true }}
                    render={({ field }) => (
                      <Input placeholder="Tur Tarihi" type="date" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
              ...
              <div className="col-6 col-md-4">
                <FormGroup id="roomNumber" label="Oda/Kapı Numarası" isFloating>
                  <Controller
                    ...
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input placeholder="Giriniz" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
              ...
              <div className="col-6 col-md-4">
                ...
                >
                  <Controller
                    ...
                    rules={{ required: true }}
                    render={({ field }) => (
                      <Input placeholder="Giriniz" type="time" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
            {Number(user.roleId)===1 && <div className="col-6 col-md-4">
              ...
              <div className="col-6 col-md-4">
                <FormGroup id="price" label="Ücret" isFloating>
                  <Controller
                    ...
                    rules={{ required: true }}
                    render={({ field }) => (
                      <Input placeholder="Giriniz" type="number" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
              ...
              <div className="col-6 col-md-4">
                <FormGroup id="ticketNumber" label="Bilet Numarası" isFloating>
                  <Controller
                    ...
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input placeholder="Tur Adı" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
                ...
                <div className="col-6 col-md-4">
                  <FormGroup id="singleCount" label="Atv Single">
                    <Controller
                      ...
                      control={control}
                      render={({ field }) => (
                        <Input type="number" placeholder="Giriniz" {...field} />
                      )}
                    />
                  </FormGroup>
                </div>
             ...
             { watch("needsTransfer") && <div className="col-6">
                <FormGroup
                  id="transferNumber"
                ...
                >
                  <Controller
                    ...
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input placeholder="Transfer Numarası" {...field} />
                    )}
                  />
                </FormGroup>
              </div>}
...
{watch("isSold")&& (watch("tourTypeId") == "12") && 
              <div className="col-6 col-md-4">
              ...
              <div className="col-12">
                <FormGroup id="note" label="Not" isFloating>
                  <Controller
                    ...
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input placeholder="Yazınız" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
...
const DynamicFragments = ({
  ...
  const deleteFragment = (id: any) => {
    ...
    let indexDeleted = fragments.findIndex(
    ...
    if (confirmation) {
      ...
      for (
        let i = 0;
        i <
        fragments.filter((fragment: { id: any }) => fragment.id !== id).length;
        i++
      ) {
        ...
        setValue(
          `customerIdentityNumber${i + 1}`,
          fragments[i]["customerIdentityNumber"]
        );
        setValue(
          `customerPhoneNumber${i + 1}`,
          fragments[i]["customerPhoneNumber"]
        );
        setValue(`customerAddress${i + 1}`, fragments[i]["customerAddress"]);
        setValue(
          `customerDateOfBirth${i + 1}`,
          fragments[i]["customerDateOfBirth"]?.split("T")[0]
        );
        ...
        (fragment: any, index: number) =>
          fragment.id !== 11 && (
            <React.Fragment key={fragment.id}>
              ...
              <div className="col-6 col-md-4">
                <FormGroup
                  id={`customerSurname${index + 1}`}
                  label="Müşteri Soyadı"
                  isFloating
                >
                  <Controller
                    name={`customerSurname${index + 1}`}
                    control={control}
                    rules={{ required: true }}
                    render={({ field }) => (
              ...
              <div className="col-6 col-md-4">
                ...
                >
                  <Controller
                    name={`customerIdentityNumber${index + 1}`}
                    control={control}
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input placeholder="Müşteri TC/PP" {...field} />
                    )}
                  />
                </FormGroup>
              </div>
              <div className="col-6 col-md-4">
                <FormGroup
                  id={`customerPhoneNumber${index + 1}`}
                  label="Müşteri Telefon"
                  isFloating
                >
                  <Controller
                    name={`customerPhoneNumber${index + 1}`}
                    control={control}
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input
             ...
             <div className="col-6 col-md-4">
                <FormGroup
                  id={`customerDateOfBirth${index + 1}`}
                  label="Müşteri Doğum Tarihi"
                  isFloating
                >
                  <Controller
                    name={`customerDateOfBirth${index + 1}`}
                    control={control}
                    rules={{ required: false }}
                    render={({ field }) => (
                      <Input
